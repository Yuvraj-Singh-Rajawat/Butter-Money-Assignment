...
**Validation**: Request and row validation is implemented using **Zod** (rows are validated and coerced before insertion; query params validated for pagination).

### File Upload & Excel Processing 🔧

- POST /api/v1/files/upload — upload a xls/xlsx/csv file (form field `file`) — returns stored file path
- POST /api/v1/files/process-excel — upload and process file, valid rows inserted into DB; invalid rows are skipped and logged (validation is implemented using Zod)
- GET /api/v1/files/getAll?page=1&limit=10&education=Degree — fetch records with pagination and optional education filter

### Sample template

A sample CSV template is included at `public/templates/sample.csv`. It contains example rows (valid, invalid, duplicate) to help exercise the validation and skipping behavior.

Example curl to test processing with the sample file:

curl -F "file=@public/templates/sample.csv" http://localhost:3000/api/v1/files/process-excel
...
> [!NOTE] > _Im working on completing this documentation to provide a more comprehensive guide for using This Api Template._
...
2. **Install dependencies**:
   ...
   > If you're using Neon (managed Postgres), prefer setting `DATABASE_URL` with the connection string and ensure `pg` and `pg-hstore` are installed.
